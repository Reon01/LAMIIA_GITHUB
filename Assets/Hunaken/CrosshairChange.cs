...
{
    [SerializeField]
    public Image Crosshair;          //クロスヘアの設定　Canvasでやってるならこのままで「Aim」を変えるだけ
    ...
    void Update()
    {
        ...
        if (Physics.Raycast(ray, out hit, 60.0f))
        {
            ...
            {
                Crosshair.color = new Color(1.0f, 0.0f, 0.0f, 1.0f);　//敵に向けたら赤色
            }
            else
            {
                Crosshair.color = new Color(1.0f, 1.0f, 1.0f, 1.0f);　//それ以外白色
            }
        }
        else
        {
